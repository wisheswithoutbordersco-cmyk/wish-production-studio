 ...
 * 
 * Supports multiple page sizes and configurable branding/page numbers.
 */
import PDFDocument from "pdfkit";

// === Upgrade 1: Branding Toggle ===
export type BrandingOption = "wishes" | "lane" | "off";

function getBrandingText(branding: BrandingOption): string | null {
  switch (branding) {
    case "wishes": return "WishesWithoutBordersCo";
    case "lane": return "Lane Digital Works";
    case "off": return null;
  }
}

// === Upgrade 4: Size Presets ===
export type PageSize = "8.5x11-portrait" | "8.5x11-landscape" | "11x14" | "16x20" | "square";

const PAGE_SIZES: Record<PageSize, { width: number; height: number }> = {
  "8.5x11-portrait": { width: 612, height: 792 },
  "8.5x11-landscape": { width: 792, height: 612 },
  "11x14": { width: 792, height: 1008 },
  "16x20": { width: 1152, height: 1440 },
  "square": { width: 720, height: 720 },
};

// Default dimensions (backward compatible)
...
export async function assemblePdf(
  ...
  const { width: PAGE_WIDTH, height: PAGE_HEIGHT } = PAGE_SIZES[pageSize] || PAGE_SIZES["8.5x11-portrait"];
  ...
  return new Promise((resolve, reject) => {
    ...
    for (const page of pages) {
      ...
      // Page number (Upgrade 2: conditional on showPageNumbers)
      ...
      // Branding footer (Upgrade 1: configurable)
      const brandText = getBrandingText(branding);
      if (brandText) {
        doc.font("Helvetica")
          .fontSize(7)
          .fillColor("#aaaaaa")
          .text(brandText, MARGIN, PAGE_HEIGHT - 20, {
